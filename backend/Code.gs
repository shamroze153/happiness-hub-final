...
function route(action, p, e) {
  switch (action) {
    ...
    case "submitDeliveryProof": return submitDeliveryProof(p);
...
function initSheetHeaders(sheet, name) {
  const headers = {
    ...
    Orders: ["order_id","buyer_name","buyer_whatsapp","product_id","product_title","amazon_order_id","screenshot_url","payment_method","payment_id","delivery_screenshot_url","notes","agent_id","seller_id","status","cashback_amount","cashback_proof_url","seller_notes","submitted_at","updated_at"],
...
// Writes a row matching whatever columns exist in the sheet header row.
// Any keys in `data` that don't match a header are silently ignored —
// this makes it safe even if extra columns haven't been added yet.
...
function getProduct(id) {
  ...
  const product = products.find(p => String(p.product_id).trim() === String(id).trim());
...
function getBuyerOrders(params) {
  ...
  const stats = {
    ...
    needs_delivery_proof: results.filter(o => (o.status === "Pending" || o.status === "Ordered") && !o.delivery_screenshot_url).length,
  };
...
function submitOrder(p) {
  ...
  logActivity(agentId, "agent", "submit_order", { order_id: order_id, product_id: product_id, buyer_whatsapp: buyer_whatsapp });
  return { success: true, order_id: order_id, message: "Order submitted successfully" };
}

// Buyer/agent marks item as delivered by uploading delivery screenshot.
// Moves status Pending/Ordered -> Delivered automatically.
function submitDeliveryProof(p) {
  const order_id = p.order_id, delivery_screenshot_url = p.delivery_screenshot_url;
  if (!order_id) return { success: false, error: "Order ID required" };
  if (!delivery_screenshot_url) return { success: false, error: "Delivery screenshot required" };
  const sheet = getSheet("Orders");
  const rowIndex = findRowIndex(sheet, 0, order_id);
  if (rowIndex === -1) return { success: false, error: "Order not found" };
  setCellByHeader(sheet, rowIndex, "delivery_screenshot_url", delivery_screenshot_url);
  const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
  const statusCol = headers.indexOf("status");
  const curStatus = statusCol > -1 ? sheet.getRange(rowIndex, statusCol + 1).getValue() : "";
  if (curStatus === "Pending" || curStatus === "Ordered" || !curStatus) {
    setCellByHeader(sheet, rowIndex, "status", "Delivered");
  }
  setCellByHeader(sheet, rowIndex, "updated_at", now());
  logActivity(p.actor_id || "buyer", p.actor_type || "buyer", "submit_delivery_proof", { order_id: order_id });
  return { success: true, message: "Delivery proof submitted" };
}
...
function updateOrderStatus(p) {
  ...
  const rowIndex = findRowIndex(sheet, 0, order_id);
  if (rowIndex === -1) return { success: false, error: "Order not found" };
  if (status) setCellByHeader(sheet, rowIndex, "status", status);
  if (seller_notes) setCellByHeader(sheet, rowIndex, "seller_notes", seller_notes);
  if (cashback_proof_url) setCellByHeader(sheet, rowIndex, "cashback_proof_url", cashback_proof_url);
  setCellByHeader(sheet, rowIndex, "updated_at", now());
...
// Adds payment_method, payment_id, delivery_screenshot_url columns to an
// existing Orders sheet if they're missing — safe to run multiple times.
function upgradeOrdersSheet() {
  const sheet = getSheet("Orders");
  const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
  const need = ["payment_method", "payment_id", "delivery_screenshot_url"];
  const missing = need.filter(h => headers.indexOf(h) === -1);
  if (missing.length === 0) { Logger.log("Orders sheet already up to date."); return; }
